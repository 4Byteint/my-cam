...
def create_rgb2gradient_dataset():
    """
    處理圖片並生成二值化遮罩
    """
    # 設定目錄路徑
    input_dir = "./imprint/dataset/images/"
    output_dir = "./imprint/dataset/masks/"
    ...
    for f in os.listdir(input_dir):
        if f.lower().endswith('.png') and 'base' in f.lower():
            base_path = os.path.join(input_dir, f)
            break
    
    if base_path is None:
        print("未找到 base 圖片（文件名需包含 'base'）")
        return
    ...
    for sample_file in os.listdir(input_dir):
        ...
        print(f"處理圖片：{sample_file}")
        
        # 使用 detect_sphere_imprints 偵測圓形
        cx, cy, r = detect_sphere_imprints(base_path, sample_path)
        if cx is None or cy is None or r is None:
            print(f"無法檢測到圓形壓痕：{sample_file}")
            continue
        ...
        # 創建二值化遮罩
        mask = np.zeros_like(sample_img[:,:,0])  # 使用第一個通道的尺寸
        cv2.circle(mask, (cx, cy), r, 255, -1)  # -1 表示填充圓
        
        # 保存遮罩圖片
        output_filename = sample_file.replace('_RGB.png', '_mask.png')
        output_path = os.path.join(output_dir, output_filename)
        cv2.imwrite(output_path, mask)
        print(f"已保存遮罩圖片：{output_path}")

# 執行函數
create_rgb2gradient_dataset()