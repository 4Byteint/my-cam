...
def batch_detect_sphere_imprints_single_base(input_dir, output_dir):
    """
    使用霍夫圓變換批量偵測球體壓痕，排除檔名中含 'base' 的圖片作為 sample 圖。
    對每張 sample 圖與 base 圖做差分、對比增強後偵測圓，畫圖後儲存至 output_dir。
    """
    os.makedirs(output_dir, exist_ok=True)

    # 搜尋 base 圖（假設只有一張 base.png）
    base_file = None
    ...
    for sample_file in os.listdir(input_dir):
        ...
        clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
        enhanced_img = clahe.apply(gray)
        gray_blurred = cv2.GaussianBlur(enhanced_img, (5, 5), 2)
        ...
        if circles is not None:
            ...
            for circle in circles[0, :]:
                x, y, r = circle
                cv2.circle(sample_img, (x, y), r, (0, 255, 0), 2)
                cv2.circle(sample_img, (x, y), 2, (0, 0, 255), 3)

            x, y, r = best_circle
            cv2.circle(sample_img, (x, y), r, (255, 0, 0), 2)
            cv2.circle(sample_img, (x, y), 2, (0, 0, 255), 3)
            print(f"[{sample_file}] ✅ 圓心 ({x}, {y})，半徑 {r}")
        else:
            print(f"[{sample_file}] ❌ 未偵測到圓形壓痕")
...
# sample_path = "./imprint/al_RGB/transform/img3_transform.png" 
# 產生 RGB 對應梯度的數據集
